...
/** Shows the five assessment choices and their completion progress. */
export default function Home() {
  const { completed = '', name: routeName } = useLocalSearchParams<{
    completed?: string;
    name?: string;
  }>();
  const { user } = useAuth();
  const name = routeName?.trim() || user?.name?.trim() || 'there';
  const [sessionCompleted, setSessionCompleted] = useState(getCompletedAssessments());
  // Refresh on every visit so a newly saved questionnaire updates progress immediately.
  useFocusEffect(
    useCallback(() => {
