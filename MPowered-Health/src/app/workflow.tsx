...
const flows: Record<string, {
  ...
  onboarding: {
    ...
    steps: [{
      ...
      title: 'Do you have a musculoskeletal (for example arthritis, back pain, gout) or chronic pain diagnosis from your doctor?',
      ...
      options: ['Arthritis', 'Ankylosing spondylitis', 'Back pain', 'Baker’s cyst', 'Bursitis', 'Foot related conditions', 'Fibromyalgia', 'Gout', 'Juvenile idiopathic arthritis and conditions', 'Lupus', 'Neck pain', 'Osteoarthritis', 'Osteoporosis', 'Paget’s disease', 'Perthes’ disease', 'Polymyalgia rheumatica', 'Psoriatic arthritis', 'Raynaud’s phenomenon', 'Reactive arthritis', 'Rheumatoid arthritis', 'Scleroderma', 'Shoulder pain', 'Sjogren’s disease'],
      multi: true,
      optional: true
    }, {
  ...
  tips: {
    ...
    steps: [{
      ...
      options: ['Understanding pain', 'Exercise and movement', 'Living well with a musculoskeletal condition', 'Relaxation and emotions']
    }]
  },
  settings: {
    eyebrow: 'SETTING',
    steps: [{
      ...
      options: ['Personal details', 'Support-person access', 'Contact permission', 'Recording permission', 'Privacy Policy', 'Terms and Conditions']
    }, {
  ...
  support: {
    ...
    steps: [{
      ...
      options: ['Appointment #1 — 24/06/2026 — John Smith — Dr Jane — General Practitioner (GP)', 'Appointment #2 — 02/08/2026 — John Smith — Dr Paul Arm — Physiotherapist']
    }, {
      title: 'Appointment #1',
  ...
  archive: {
    ...
    steps: [{
      ...
      options: ['Archived Appointment #1 — 24/10/2025 — Dr Jane', 'Archived Appointment #2 — 02/08/2025 — Dr Paul Arm']
    }, {
  ...
  appointment: {
    ...
    steps: [{
      ...
      options: ['General Practitioner', 'Physiotherapist', 'Rheumatologist', 'Osteopath', 'Pain Medicine Specialist', 'Orthopaedic surgeon', 'Occupational Therapist']
    }, {
...
const meds = ['Perindopril arginine 5 mg — Once daily', 'Candesartan 16 mg — Once daily', 'Amlodipine 5 mg — Once daily', 'Vitamin D3 1000 IU — Once daily', 'Raloxifene 60 mg — Once daily'];
function Prescriptions() {
  ...
  if (adding) return <Shell title="Add prescription" onBack={() => setAdding(false)}><Field label="Medication name" value={name} set={setName} /><Field label="Strength" value={strength} set={setStrength} /><Choice title="Strength unit" options={['mg', 'g', '%', 'μg', 'iu']} value={[unit]} pick={v => setUnit(v)} /><Choice title="Form" options={['Tablet', 'Capsule', 'Liquid', 'Drops', 'Injections', 'Spray', 'mL', 'Patches']} value={[form]} pick={v => setForm(v)} /><Choice title="Repeat every" options={['hour', 'day', 'week', 'month']} value={[repeat]} pick={v => setRepeat(v)} /><ActionButton label="Save prescription" onPress={() => {
      if (name.trim()) {
  ...
  return <Shell title="My Prescriptions" onBack={() => router.replace('/explore')}><Text style={s.copy}>{list.length ? 'Your prescribed medications' : 'Prescription list is empty'}</Text><View style={s.list}>{list.map((m, i) => <View key={`${m}${i}`} style={s.med}><Text style={s.medText}>{m}</Text><Pressable onPress={() => setList(v => v.filter((_, x) => x !== i))}><Text style={s.remove}>Remove</Text></Pressable></View>)}</View><ActionButton label="Add prescription" onPress={() => setAdding(true)} /></Shell>;
}
function Shell({
  title,
  children,
  ...
  return <SafeAreaView style={s.safe} edges={['top']}><MhaHeader /><ScrollView contentContainerStyle={s.content}><Pressable onPress={onBack}><Text style={s.back}>‹  Back</Text></Pressable><Text style={s.title}>{title}</Text>{children}</ScrollView></SafeAreaView>;
}
function Field({
  label,
  value,
  ...
  if (label === 'Appointment date') {
    ...
    const onDateChange = (_event: DateTimePickerEvent, date?: Date) => {
      if (Platform.OS === 'android') {
        ...
        if (date) set(date.toLocaleDateString('en-AU', { day: '2-digit', month: '2-digit', year: 'numeric' }));
...
function Choice({
  ...
  return <View style={s.choiceWrap}>{title ? <Text style={s.choiceTitle}>{title}</Text> : null}{options.map(o => {
      const on = value.includes(o);
      return <Pressable key={o} onPress={() => pick(o)} style={[s.choice, on && s.choiceOn]}><Text style={[s.choiceText, on && s.choiceTextOn]}>{o}</Text><View style={[multi ? s.square : s.circle, on && s.mark]}><Text style={s.tick}>{on ? '✓' : ''}</Text></View></Pressable>;
    })}</View>;
}
function AppointmentQuestions({options,value,pick,customQuestion,setCustomQuestion}:{options:string[];value:string[];pick:(v:string)=>void;customQuestion:string;setCustomQuestion:(value:string)=>void}) {
  const groups=[
  ...
  return <View style={{marginTop:18}}>
    {groups.map(group=><Choice key={group.title} title={group.title} options={group.items} value={value} pick={pick} multi/>)}
    <View style={s.choiceWrap}>
      <Text style={s.choiceTitle}>Other</Text>
      <TextInput multiline value={customQuestion} onChangeText={setCustomQuestion} placeholder="Type your question" placeholderTextColor="#81798A" style={[s.input,{minHeight:92,backgroundColor:'#fff'}]}/>
    </View>
  </View>;
}
function CompactSelect({title,options,value,pick}:{title:string;options:string[];value:string[];pick:(v:string)=>void}) {
  const [open,setOpen]=useState(false);
  return <View style={s.selectWrap}><Text style={s.selectLabel}>{title}</Text><Text style={s.optional}>optional</Text><Pressable accessibilityRole="button" accessibilityState={{expanded:open}} onPress={()=>setOpen(v=>!v)} style={[s.selectField,open&&s.selectFieldOpen]}><Text style={[s.selectValue,!value[0]&&s.datePlaceholder]}>{value[0]||'Select healthcare practitioner service'}</Text><Text style={s.selectChevron}>{open?'⌃':'⌄'}</Text></Pressable>{open?<View style={s.selectMenu}>{options.map(option=><Pressable key={option} onPress={()=>{pick(option);setOpen(false)}} style={[s.selectOption,value[0]===option&&s.selectOptionOn]}><Text style={[s.selectOptionText,value[0]===option&&s.selectOptionTextOn]}>{option}</Text>{value[0]===option?<Text style={s.selectTick}>✓</Text>:null}</Pressable>)}</View>:null}</View>;
}
export default function Workflow() {
  const {
    flow = 'onboarding', name: routeName = 'Jane', step: initialStep = '0', fresh, resume, doctor:resumeDoctor, date:resumeDate, service:resumeService, support:resumeSupport, customQuestion:resumeCustomQuestion, questions:resumeQuestions
  ...
  useEffect(()=>{if(flow!=='appointment')return;setStep(Number(initialStep)||0);if(resume){setValues({0:resumeService&&resumeService!=='Not added'?[resumeService]:[],2:resumeQuestions?JSON.parse(resumeQuestions) as string[]:[]});setFields({'0-Appointment date':resumeDate??'','0-Doctor’s name':resumeDoctor??'','1-Support person name':resumeSupport==='Not added'?'':resumeSupport??'','2-Other question':resumeCustomQuestion??''})}else{setValues({});setFields({})}setRecordedUri(undefined)},[flow,initialStep,fresh,resume,resumeDoctor,resumeDate,resumeService,resumeSupport,resumeCustomQuestion,resumeQuestions]);
  ...
  const pick = (v: string) => {
    ...
    setValues(a => ({
      ...a,
      [step]: current.multi ? selected.includes(v) ? selected.filter(x => x !== v) : [...selected, v] : [v]
    }));
  };
  ...
  const next = () => {
    ...
    if (flow === 'appointment' && current.title === 'Add Questions for My Appointment') {
      ...
      router.push({pathname:'/appointment-review',params:{mode:'plan',doctor:fields['0-Doctor’s name']?.trim()||'Healthcare practitioner',date:fields['0-Appointment date']||'Date not specified',service:values[0]?.[0]||'Not added',support:fields['1-Support person name']?.trim()||'Not added',customQuestion:customQuestion||'',questions:JSON.stringify(selected),questionData:JSON.stringify(groupedQuestions)}});
      return;
    }
    if (flow === 'appointment' && current.title === 'Review My Appointment Plan') {
      addAppointment({doctor:fields['0-Doctor’s name']?.trim()||'Healthcare practitioner',date:fields['0-Appointment date']||'Date not specified',service:values[0]?.[0]||'Health service not specified'});
      router.replace('/care');
      return;
    }
    if (flow === 'tips' && step === 0 && selected.length) {
      const urls: Record<string, string> = {
        ...
        'Living well with a musculoskeletal condition': 'https://muscha.org/living-well-with-a-musculoskeletal-condition',
...
const s = StyleSheet.create({
  ...
  content: {
    width:'100%',maxWidth:680,alignSelf:'center',paddingHorizontal:24,paddingTop:20,
    paddingBottom: 120
  },
  back: {
    fontSize: 14,
  ...
  optional: { fontSize: 12, lineHeight: 17, fontWeight: '600', color: palette.muted, marginTop: 4, marginBottom: 8 },
  selectField: { minHeight: 56, borderRadius: 17, borderWidth: 1, borderColor: palette.line, backgroundColor: '#fff', paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10 },
  selectFieldOpen: { borderColor: palette.secondary, borderBottomLeftRadius: 8, borderBottomRightRadius: 8 },
  selectValue: { flex: 1, fontSize: 14, lineHeight: 20, color: palette.text },
  selectChevron: { fontSize: 20, lineHeight: 20, fontWeight: '800', color: palette.text },
  selectMenu: { marginTop: 6, borderRadius: 16, borderWidth: 1, borderColor: palette.line, backgroundColor: '#fff', overflow: 'hidden' },
  selectOption: { minHeight: 50, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: palette.line },
  selectOptionOn: { backgroundColor: '#D8C7FA' },
  selectOptionText: { flex: 1, fontSize: 14, lineHeight: 20, color: palette.text },
  ...
  dateModalTitle: { fontSize: 18, lineHeight: 24, fontWeight: '800', color: palette.text, textAlign: 'center' },
  dateModalActions: { flexDirection: 'row', gap: 10, marginTop: 4 },
  dateModalButton: { flex: 1, minHeight: 48, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  ...
  voiceButton:{minHeight:56,borderRadius:18,borderWidth:1,borderColor:palette.accent,backgroundColor:'#F7F4FC',paddingHorizontal:18,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10},
  ...
  recorded:{fontSize:12,fontWeight:'700',color:palette.success,textAlign:'center'},
  list: {
    gap: 12,
    marginVertical: 18
  },
