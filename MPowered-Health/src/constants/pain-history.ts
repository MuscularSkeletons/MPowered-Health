  ...
  const write = writes
    ...
    .then(async () => {
      ...
      const { error } = await supabase
        ...
        .insert([
          {
            pain_location: record.areas,
            pain_characteristics: record.answers[1] ?? [],
            current_pain: record.current,
            average_pain: record.average,
            mildest_pain: record.mildest,
            worst_pain: record.worst,
          },
        ]);
