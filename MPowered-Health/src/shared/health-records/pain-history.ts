...
let storageKey = 'mpowered:pain-history:v1';
let generation = 0;

/** Select an account without exposing another account’s saved assessments. */
export function setPainHistoryAccount(userId: string) {
  generation += 1;
  storageKey = `mpowered:pain-history:v1:${encodeURIComponent(userId)}`;
  records = [];
}
...
export async function savePainAssessment(answers: AssessmentAnswers, now = new Date()) {
  ...
  const write = writes
    ...
    .then(async () => {
      ...
      await AsyncStorage.setItem(requestKey, JSON.stringify(updated));
      if (requestGeneration === generation) records = updated;
      return clone(record);
    });
  writes = write;
  return write;
