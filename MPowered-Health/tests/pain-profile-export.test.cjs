...
function load(name, mocks = {}, globals = {}) {
  ...
  vm.runInNewContext(code, { exports, Error, ...globals, require: (id) => {
    ...
    if (id === './my-health/pain-profile/sharing/report') return load('my-health/pain-profile/sharing/report');
...
test('HTML and text contain every section, date and answer; HTML escapes user content', () => {
  const { profileReportHtml, profileReportText } = load('my-health/pain-profile/sharing/report');
...
test('native printing receives the full report and sharing receives a PDF file', async () => {
  const calls = [];
  const api = load('shared/export/document-export', {
    'expo-print': {
  ...
  await api.printHtml(load('my-health/pain-profile/sharing/report').profileReportHtml(report));
  assert.equal(await api.shareDocument({ title: 'My Pain Profile', html: load('my-health/pain-profile/sharing/report').profileReportHtml(report), text: load('my-health/pain-profile/sharing/report').profileReportText(report) }), 'done');
...
test('native sharing falls back to the complete text when file sharing is unavailable', async () => {
  ...
  await api.shareDocument({ title: 'My Pain Profile', html: load('my-health/pain-profile/sharing/report').profileReportHtml(report), text: load('my-health/pain-profile/sharing/report').profileReportText(report) });
...
test('PDF generation failures propagate so the UI can show an error and retry', async () => {
  ...
  await assert.rejects(api.shareDocument({ title: 'My Pain Profile', html: load('my-health/pain-profile/sharing/report').profileReportHtml(report), text: load('my-health/pain-profile/sharing/report').profileReportText(report) }), /Disk full/);
});
...
test('web sharing uses the report text, with unsupported, rejected and cancelled outcomes', async () => {
  let shared;
  const navigator = {};
  const api = load('shared/export/document-export.web', {}, { navigator });
  assert.equal(await api.shareDocument({ title: 'My Pain Profile', html: load('my-health/pain-profile/sharing/report').profileReportHtml(report), text: load('my-health/pain-profile/sharing/report').profileReportText(report) }), 'copy');
...
test('web printing prints the isolated HTML document and cleans up after the dialog', async () => {
  ...
  const api = load('shared/export/document-export.web', {}, {
...
test('web print failure rejects and removes the temporary document', async () => {
  ...
  await assert.rejects(api.printHtml(load('my-health/pain-profile/sharing/report').profileReportHtml(report)), /unavailable/);
  assert.ok(removed);
});
...
test('copy uses clipboard, falls back to selection, and reports failures honestly', async () => {
  ...
  assert.equal(await api.copyText('Report'), true);
  assert.equal(copied, 'Report');
  navigator.clipboard.writeText = async () => { throw new Error('Denied'); };
  assert.equal(await api.copyText('Fallback report'), true);
  assert.equal(field.value, 'Fallback report');
  assert.ok(selected && removed && restored);
  legacyResult = false;
  assert.equal(await api.copyText('Manual copy'), false);
});
...
test('dismissing iOS printing is quiet while printer failures still propagate', async () => {
  ...
  await assert.rejects(api.printHtml(load('my-health/pain-profile/sharing/report').profileReportHtml(report)), /Printer unavailable/);
});
