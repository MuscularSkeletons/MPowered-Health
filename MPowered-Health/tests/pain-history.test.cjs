...
test('weeks 1 and 3 share one exact combination; week 2 stays separate', async () => {
  ...
  const history = load('shared/health-records/pain-history');
...
test('each area combination is order-independent and never matches subsets', () => {
  const history = environment().load('shared/health-records/pain-history');
...
test('assessment snapshots survive reload and later answer edits', async () => {
  ...
  const reloaded = environment(env.storage).load('shared/health-records/pain-history');
...
test('filtered PDFs contain only selected assessments and use actual metrics', async () => {
  ...
  const groups = history.groupPainHistory(history.getPainHistory());
  const pdf = load('my-health/health-records/report');
...
test('signed-in accounts have separate history and switching discards pending loads', async () => {
  const env = environment();
  const history = env.load('shared/health-records/pain-history');
  history.setPainHistoryAccount('alice');
  await history.savePainAssessment(answers(['Back']), date(1));
  const loading = history.loadPainHistory();
  history.setPainHistoryAccount('bob');
  await loading;
  assert.equal(history.getPainHistory().length, 0);
  await history.loadPainHistory();
  assert.equal(history.getPainHistory().length, 0);
  history.setPainHistoryAccount('alice');
  await history.loadPainHistory();
  assert.equal(history.getPainHistory().length, 1);
...
test('queued saves cannot cross accounts', async () => {
  const history = environment().load('shared/health-records/pain-history');
  history.setPainHistoryAccount('alice');
  const saving = history.savePainAssessment(answers(['Back']), date(1));
  history.setPainHistoryAccount('bob');
